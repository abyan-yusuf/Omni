...
exports.updateSubCategory = async (req, res) => {
  try {
    ...
    if (!parentCat) {
      return res
        .status(404)
        .send({ message: "Please enter a parent category" });
    }
...
exports.deleteSubCategory = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .send({ message: "SubCategory deleted", subCategory: deleteSubCategory });
  } catch (error) {
    console.error(error);
  }
};
...
exports.getSubCategoryById = async (req, res) => {
  try {
    const { subid } = req.params;
    const singleSubCategory = await SubCategory.findById(subid)
      .select("-image")
      .populate("parentCat");
...
exports.getSubCategoriesByParentCat = async (req, res) => {
  try {
