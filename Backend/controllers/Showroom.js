...
exports.createShowroom = async (req, res) => {
  try {
    ...
    if (!latitude || !longitude)
      return res.status(400).send({ message: "Coordinates are required" });

    const existingShowroom = await Showroom.findOne({ code });
    if (existingShowroom)
      return res.status(400).send({ message: "Showroom already exists" });
...
exports.updateShowroom = async (req, res) => {
  try {
    ...
    if (!existingShowroom)
      return res.status(400).send({ message: "Showroom not found" });
...
exports.getFilteredShowrooms = async (req, res) => {
  try {
