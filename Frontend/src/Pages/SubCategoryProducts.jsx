...
const SubCategoryProducts = () => {
  ...
  const getSubCategory = async () => {
    try {
      const { data } = await axios.get(
        `https://omni-yxd5.onrender.com/api/v1/sub-categories/single/${sid}`
      );
      setSubCategory(data);
    } catch (error) {
      console.log(error);
  ...
  const getProducts = async () => {
    try {
      const { data } = await axios.get(
        `https://omni-yxd5.onrender.com/api/v1/products/sub-category/${sid}`
      );
      setProducts(data);
    } catch (error) {
      console.log(error);
  ...
  return (
    <>
      {products.length && subCategory ? (
        <Layout><div className="breadcrumbs text-sm mt-5 ml-5">
                  <ul>
                    <li>
                      <Link to={"/"}>Home</Link>
                    </li>
